...
import { TeamTheme, PieceType, PieceColor } from '../types';
...
const ChessBoard: React.FC<ChessBoardProps> = ({ isOnFire = false }) => {
  const {
    chess,
    state,
    displayedBoard,
  ...
  const displayRows = boardOrientation === 'white' ? [0, 1, 2, 3, 4, 5, 6, 7] : [7, 6, 5, 4, 3, 2, 1, 0];
  const displayCols = boardOrientation === 'white' ? [0, 1, 2, 3, 4, 5, 6, 7] : [7, 6, 5, 4, 3, 2, 1, 0];

  const isBigAttack = boardEffect?.variant === '3x3';
  const isCrit = boardEffect?.type === 'crit';

  const themeColors = BOARD_THEMES[whiteTheme as TeamTheme];
      ...
      <div className="w-full h-full bg-[#2a2018] p-1 rounded-lg shadow-inner">
        <div className="w-full h-full grid grid-rows-8 grid-cols-8 border-2 border-[#5c4a3d] relative overflow-hidden z-10 rounded shadow-2xl">
          {displayRows.map((r) => (
            displayCols.map((f) => {
              const piece = board[r][f];
              const squareLabel = `${files[f]}${ranks[r]}`;
              const isValid = validDestinations.includes(squareLabel);
              ...
              return (
                <div key={squareLabel} className="relative w-full h-full">
                  <BoardSquare
                    ...
                    onClick={() => handleSquareClick(squareLabel as any)}
                  />
